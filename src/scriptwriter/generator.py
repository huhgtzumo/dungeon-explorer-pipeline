...
- 探索者不攜帶也不使用任何拍攝設備（相機、手機、GoPro），畫面就是探索者的眼睛
- 禁止安排「回看照片」「檢查手機螢幕」「回放影片」等動作
...
- 探索者不攜帶也不使用任何拍攝設備（相機、手機、GoPro、攝影機），畫面即探索者的眼睛視角
- 不安排「回看照片」「檢查螢幕」「回放畫面」等需要第二台拍攝設備的動作
- 每個場景的 visual_description 只描述探索者眼前的環境，不描述任何螢幕或顯示器上的畫面
